...
export const FloatingToolbar: React.FC<FloatingToolbarProps> = ({
  ...
  React.useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.dropdown-container') && !target.closest('.toolbar-dropdown')) {
    ...
    <>
          ...
          <div className="flex items-center gap-3 min-w-max">
            {/* Drawing Tools */}            <div className="flex items-center gap-1 bg-gray-50/80 rounded-xl p-1">
              {tools.map((tool) => {
                ...
                return (
                  <motion.button
                    key={tool.id}
                    ref={tool.id === 'eraser' ? eraserButtonRef : undefined}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                      if (tool.id === 'eraser') {
                        const rect = e.currentTarget.getBoundingClientRect()
                        setEraserSizePickerPosition({ 
                          x: rect.left, 
                          y: rect.bottom + 8 
                        })
                        setShowEraserSizePicker(!showEraserSizePicker)
                        setShowColorPicker(false)
                        setShowLineWidthPicker(false)
                    ...
                    title={tool.id === 'eraser' ? `${tool.label} (Click for size)` : tool.label}
                  >
            ...
            <div className="relative dropdown-container">
              <motion.button
                ref={colorButtonRef}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}                onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                  const rect = e.currentTarget.getBoundingClientRect()
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {colors.map((color) => (
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}                onMouseDown={(e: React.MouseEvent) => {
                  e.preventDefault()
                  onColorChange(color)
                  setTimeout(() => setShowColorPicker(false), 50)
                }}
                className={cn(
      ...
      {showLineWidthPicker && typeof window !== 'undefined' && createPortal(
        ...
        >
          {lineWidths.map((width) => (
            <motion.button
              key={width}
              whileHover={{ scale: 1.05 }}              whileTap={{ scale: 0.95 }}              onMouseDown={(e: React.MouseEvent) => {
                e.preventDefault()
                onLineWidthChange(width)
                setTimeout(() => setShowLineWidthPicker(false), 50)
              }}
              className={cn(
      ...
      {showEraserSizePicker && typeof window !== 'undefined' && createPortal(
        <motion.div
          ...
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.9, y: -10 }}
          className="toolbar-dropdown fixed bg-white rounded-xl shadow-2xl border border-gray-200 p-3 flex flex-col gap-2 z-[99999] min-w-[120px]"
          style={{
            left: eraserSizePickerPosition.x,
            top: eraserSizePickerPosition.y,
          }}
        >
          <div className="text-xs font-medium text-gray-700 mb-1 text-center">Eraser Size</div>
          {lineWidths.map((width) => (
            <motion.button
              key={width}
              whileHover={{ scale: 1.05 }}              whileTap={{ scale: 0.95 }}              onMouseDown={(e: React.MouseEvent) => {
                e.preventDefault()
                onLineWidthChange(width)
                setTimeout(() => setShowEraserSizePicker(false), 50)
              }}
              className={cn(
                'flex items-center justify-center p-3 rounded-lg transition-all min-h-[40px]',
                lineWidth === width 
            ...
            >
              <div 
                className="bg-white rounded-full border border-gray-300 shadow-md"
                style={{ 
                  width: `${Math.max(width * 2, 4)}px`, 
                  height: `${Math.max(width * 2, 4)}px`,                  boxShadow: '0 2px 4px rgba(0, 0, 0, 0.3), inset 0 1px 2px rgba(0, 0, 0, 0.1)'
                }}
              />
            </motion.button>
          ))}
